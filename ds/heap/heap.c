...
typedef enum {UP, DOWN} sift_direction_t;
typedef enum {LEFT_DOWN, RIGHT_DOWN, SIFT_UP, NONE} sift_status_t;
typedef enum {LEFT, RIGHT} direction_t;
const int FAIL = -1;
const size_t VECTOR_INIT_CAPACITY = 1;
const size_t VECTOR_RESERVE_CAPACITY = 100;
/*************************************************************************
*								 										 *
...
static int GetIndexToRemove(heap_t *heap,
							...
							void *data_to_remove);
static void Sift(sift_direction_t sift_dir, size_t index, heap_t *heap);
static void SiftOnRemove(heap_t *heap, size_t idx_to_remove);
static sift_direction_t SiftDirection(heap_t *heap, size_t index);
static int IsPriorityGreaterThanParent(const heap_t *heap, size_t index);
static int IsPriorityGreater(const heap_t *heap, size_t index, direction_t dir);
static int HasChild(const heap_t *heap, size_t index, direction_t dir);
static direction_t ChildSiftDirection(heap_t *heap, size_t index);
static int IsLeftChildGreaterPriority(heap_t *heap, size_t parent_idx);
static direction_t GetDirectionOfChildWithHigherPriority(const heap_t *heap,
													   size_t parent_idx);
static int IsChild(size_t index);
static size_t GetChildIdx(size_t index, direction_t dir);
...
/* returns positive number if index exists. FAIL otherwise. */
static int GetIndexToRemove(heap_t *heap,
							int (*is_match)(const void *heap_data,	
							const void *user_data),
							void *data_to_remove)
{
	...
	for (i = 0; (size_t) i < HeapSize(heap); ++i)
	{
		...
		if (is_match(heap_data, data_to_remove))
		{
			return i;
		}
	}
...
static void SiftOnRemove(heap_t *heap, size_t idx_to_remove)
{
	sift_status_t sift_stat = SiftDirection(heap, idx_to_remove);
...
static direction_t ChildSiftDirection(heap_t *heap, size_t index)
{
	...
	{
		direction_t dir = GetDirectionOfChildWithHigherPriority(heap, index);
